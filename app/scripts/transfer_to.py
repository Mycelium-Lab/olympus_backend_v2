...
async def getTransferTo(timestamp_start, period):
    ...
    queryString = f"""query getTransfer {{
      t_from1:transfers(orderBy: timestamp, first: 1000, where: {{to: "0x9272B16e278051Ed961886E67bF31b45e2D8CB66"}}) {{
        to
        amount
        timestamp
      }}
      t_from2:transfers(orderBy: timestamp, skip: 1000, first: 1000, where: {{to: "0x9272B16e278051Ed961886E67bF31b45e2D8CB66"}}) {{
        to
        amount
        timestamp
      }}
      t_from3:transfers(orderBy: timestamp, skip:1000, first: 1000, where: {{to: "0x9272B16e278051Ed961886E67bF31b45e2D8CB66"}}) {{
        to
        amount
        timestamp
      }}
...
if __name__ == "__main__":
    res = getTransferTo(timestamp_start, days)
    print(res)
